    ...
    function bind(tmplItem, elem) {
        ...
        if (elem.childNodes.length == 1 && elem.childNodes[0].nodeType == 3) {
            ...
            if (binding) {
				...
				converterB = function (value, source, target) {
				...
				eval('mapping.' + binding.field + ' = { convert: converterB }');
        ...
        if (binding) {
            ...
            if (rInputSelectTextarea.test(elem.nodeName)) {
                    ...
                    // Linked to "value", bind as single value for radio, set of values for checkbox
                        ...
                        if (elem.type.toLowerCase() == "radio") {
                            ...
                            wrappedElem.bind("change", function (e) {
								...
								//TODO: Remove eval
                                $(binding.data).data(eval('(' + binding.field + ')'), currentValue);
                            });

                            // Update the radio button when the field value changes
                            $(binding.data).bind("changeData", function (e, field, value) {
                                if (field == eval('(' + binding.field + ')')) {
                        ...
                        // Check-boxes
                            ...
                            wrappedElem.bind("change", function (e) {
                                ...
                                // TODO: Remove eval
								var fieldValue = eval('(binding.data.' + binding.field + ')' );
                ...
                // All other input controls
					...
					// TODO: Remove eval
					var extVar = $.extend({ name: elem.name }, { convert: binding.converter.convertBack, convertBack: binding.converter.convert });
					var a = binding.field.match(/([A-Z]+)\[\d+\]/g);
					if ( a != null)
					{    b = mapping;
						for( i = 0; i < a.length; i++)
						{	var arr = a[i].match(/[A-Z]+/);
							b[arr] = []; // add the LPC
							var idx = a[i].match(/\d+/g);
							if( a[i+1] !== undefined )
								b[arr][idx] = [];
							else
								b[arr][idx] = {};
							b = b[arr][idx] ; // Reference LPC[x]
						}
					}
            ...
            // Otherwise, setup one-way linking
				...
				var converterA = function (value, source, target) {
					if (binding.converter && binding.converter.convert) {
						value = binding.converter.convert(value, source, target)
							if (value === undefined)
								return;
				...
				eval('mapping.' + binding.field + ' = { convert: converterA }');
